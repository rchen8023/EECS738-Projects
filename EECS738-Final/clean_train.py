...
import cv2
import argparse
from easyCNN import CNN
from data_loader import GetLoader

parser = argparse.ArgumentParser()
parser.add_argument("data", type=str, default='cifar', help='Dataset to use.')
...
def save_img(dataset):
    for i in range(10):
        one_img = dataset[1000*i][0].clone().numpy()
        ...
        reshape_img = reshape_img * 255
        cv2.imwrite('./saved_img/'+DATASET_NAME+'example'+str(i)+'.png', reshape_img, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
    return
...
def test(model, epoch, mode, dataloader):
    ...
    if mode == 'normal':
        ...
        print('Epoch: [{}/{}], atk suc on {} dataset: {:.4f}%'.format(epoch, N_EPOCH, DATASET_NAME, accu, ))
    else:
        return
    return accu
...
def train(model, optimizer, dataloader, test_dataloader, poison_dataloader):
    ...
    for epoch in range(1, N_EPOCH + 1):
        ...
        while i < len_dataloader + 1:
            ...
            optimizer.zero_grad()
            img, label = data_source[0], data_source[1]
        ...
        if acc + acc2 > best_acc and not args.p:
            # torch.save(model.state_dict(), './saved_models/'+DATASET_NAME+'_clean.pth')
            best_acc = acc + acc2

        elif acc + acc2 > best_acc and args.p and args.s:
            torch.save(model.state_dict(), './saved_models/' + DATASET_NAME + '_poison'
                       +str(args.r)+'_s'+str(args.d)+str(args.d)+'_label'+str(args.t)+'.pth')
            best_acc = acc + acc2

        elif acc + acc2 > best_acc and args.p and args.s == False:
            torch.save(model.state_dict(), './saved_models/' + DATASET_NAME + '_poison'
...
if __name__ == '__main__':
    ...
    poison_dataset = load_test_data()
    poison_dataloader = process_data(poison_dataset, mode='test')
    model = CNN(CLASS_NUM).to(DEVICE)
    if args.s == False:
        model.load_state_dict(torch.load('./saved_models/'+DATASET_NAME+'_clean.pth'), strict=True)
    for param in model.parameters():
        param.requires_grad = True
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    train(model, optimizer, train_dataloader, test_dataloader, poison_dataloader)
    acc = test(model, 0, 'test', poison_dataloader)
    acc2 = test(model, 0, 'normal', test_dataloader)
